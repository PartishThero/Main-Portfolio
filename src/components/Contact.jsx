...
// Class names
const labelClassName = 'font-mono text-[0.7rem] uppercase';
...
export default function Contact() {
  ...
  const handleButtonEnter = e => { e.currentTarget.style.transform = 'translate(0, -3px)'; e.currentTarget.style.boxShadow = '4px 8px 0px var(--bg-surface)'; };
  const handleButtonLeave = e => { e.currentTarget.style.transform = 'translate(0, 0)';    e.currentTarget.style.boxShadow = '4px 4px 0px var(--bg-surface)'; };
  const handleButtonDown  = e => { e.currentTarget.style.transform = 'translate(2px, 2px)'; e.currentTarget.style.boxShadow = '2px 2px 0px var(--bg-surface)'; };
  const handleButtonUp    = e => { e.currentTarget.style.transform = 'translate(0, 0)';    e.currentTarget.style.boxShadow = '4px 4px 0px var(--bg-surface)'; };

  // Form submit — sends to Web3Forms API
          ...
          {sent ? (
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
              ...
              <div className="flex flex-col gap-2">
                <label htmlFor="contact-name" className={labelClassName} style={formInputStyle}>Name</label>
                <input id="contact-name" name="name" type="text" placeholder="Your name" value={form.name} {...baseInputProps} />
              </div>

              {/* Email */}
              <div className="flex flex-col gap-2">
                <label htmlFor="contact-email" className={labelClassName} style={formInputStyle}>Email</label>
                <input id="contact-email" name="email" type="email" placeholder="your@email.com" value={form.email} {...baseInputProps} />
              </div>

              {/* Message */}
              <div className="flex flex-col gap-2">
                <label htmlFor="contact-message" className={labelClassName} style={formInputStyle}>Message</label>
                <textarea id="contact-message" name="message" placeholder="What should we work on? :3" value={form.message} rows={5} {...baseInputProps} />
              </div>

              {error && (
                <p className="font-mono text-xs" style={{ color: 'var(--accent-main)' }}>
